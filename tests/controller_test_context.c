...
static uint64_t add_text(AppState *state, const char *text) {
...
static void test_contextual_add_defaults(void) {
    ...
    const uint64_t all = add_text(&state, "all task");
    ...
    const uint64_t today = add_text(&state, "today task");
    ...
    const uint64_t upcoming = add_text(&state, "upcoming task");
    ...
    const uint64_t completed = add_text(&state, "completed task");
